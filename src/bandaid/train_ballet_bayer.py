...
turned on (a fraction of samples get a per-CFA-channel star+sky colour, then the real
`bandaid.bayer_balance_image` is run on the padded frame and the centre is cropped), so
...
import eloy.ballet.training as T
from eloy.ballet.model import CNN, load_weights_file
from bandaid import bayer_balance_image

SIZE = 15
WARM = os.environ.get(
    "BALLET_WARM", "/Users/mattcraig/development/astronomy/eloy/ballet_realistic_15x15.npz"
)
OUT = os.environ.get(
    "BALLET_OUT",
...
def gen_fn(n):
    ...
    return gen.random_realistic_label(
        n, bayer_frac=BAYER_FRAC, balance_fn=bayer_balance_image
    )
...
def main():
    ...
    lr = 1e-4
    state = T.TrainState.create(apply_fn=model.apply, params=params, tx=optax.adamw(lr))
    X_train, y_train = gen_fn(TRAIN_SIZE)
    lr_drops = {EPOCHS * 3 // 5: 1e-5}  # one drop at 60% through

    print(f"fine-tune {EPOCHS} epochs, bayer_frac={BAYER_FRAC}, LR {lr:.0e} -> {list(lr_drops.values())}")
    t0 = time.time()
    for epoch in range(EPOCHS):
        for batch in T.get_batches(X_train, y_train, BATCH):
            state, loss = T.train_step(state, batch)
        if epoch in lr_drops:
            lr = lr_drops[epoch]
            state = T.TrainState.create(
                apply_fn=model.apply, params=state.params, tx=optax.adamw(lr)
            )
        if epoch % 10 == 0:
            rmse = T.eval_step(state.params, test_batch)
            print(
    ...
    for i in range(10):
        for batch in T.get_batches(Xa, ya, BATCH):
            state, loss = T.train_step(state, batch)
    ...
    np.savez(OUT, **T.params_to_flat_dict(best))
