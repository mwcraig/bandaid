...
case-insensitively; an optional `name` column is accepted and ignored — it
is input-side documentation only and is not written to the `.star` output
(see [Understanding the output](outputs.md)). Values are interpreted as
ICRS degrees: for an ECSV table carrying column units, an angular unit
(e.g. `hourangle`) is converted, while a non-angular unit is a clean error.
A masked/blank or non-finite `ra`/`dec`, or an RA outside `[0, 360)`, is
also a clean error.

Forced targets are **not checked against the Gaia magnitude/contamination
...
have already taken any contamination it causes into account (see
